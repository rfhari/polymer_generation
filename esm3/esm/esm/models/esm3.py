...
class EncodeInputs(nn.Module):
    ...
    def __init__(self, d_model: int):
        ...
        # Sequence
        self.sequence_embed = nn.Embedding(500, d_model)
    ...
    def forward(
        ...
        print("sequence_tokens from esm3.py:", sequence_tokens.requires_grad)

        sequence_embed = self.sequence_embed(sequence_tokens)
        print("sequence_embed from esm3.py:", sequence_embed.requires_grad)
...
class OutputHeads(nn.Module):
    def __init__(self, d_model: int):
        super().__init__()
        self.sequence_head = RegressionHead(d_model, 500)
...
class ESM3(nn.Module, ESM3InferenceClient):
    ...
    def forward(
        ...
        *,
        ...
        """
        # Reasonable defaults:
        print("im in the local esm class")
        try:
            L, device = next(
                (x.shape[1], x.device)
                for x in [
    ...
    def batch_generate(
        ...
        if isinstance(inputs[0], ESMProtein):
            print(f"within this instance {inputs[0]}")
            print("return from iterative function:", iterative_sampling_raw(self, inputs, configs))
